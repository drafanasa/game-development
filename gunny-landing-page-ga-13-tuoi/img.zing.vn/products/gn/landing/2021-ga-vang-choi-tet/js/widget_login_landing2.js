...
function forceDownload() {
    if (typeof window.location.href.split('?')[1] != "undefined") {
        ...
        if(logged) {
            download("https://gunnyvn.vcdn.vn/Gunny_Launcher.exe");
        }
    }
}
...
function dkn_callback(cbdata, acn, zid, uin, autcode, obj) {
    ...
    var urlencode = encodeURIComponent(location.protocol + '//' + siteUrl + '/launcher');
    param = "cb=" + urlencode + "&" + param; 
    if (zid > 0) {
        var register_url = "http://id.gn.zing.vn/quick-play" + "?" + param;
        var login_url;
        if ($('#checklogin').attr('rel')) {
            login_url = "http://id.gn.zing.vn/quick-play" + $('#checklogin').attr('rel') + "&" + param;
        } else {
            login_url = "http://id.gn.zing.vn/quick-play" + "?" + param;
        }
